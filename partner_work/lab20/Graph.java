...
public class Graph implements Iterable<Integer> {
    ...
    public ArrayList<Integer> shortestPath(int start, int stop) {
        ...
        while (!pq.isEmpty()) {
            int v = (int) (pq.poll() & Integer.MAX_VALUE);
            if (!visited[v]) {
                visited[v] = true;
                for (int i = 0; i < adjLists[v].size(); i++) {
                    Edge nextEdge = adjLists[v].get(i);
                    if (!visited[nextEdge.to]) {
                        int possibleD = dist[v] + nextEdge.weight;
                        if (possibleD < dist[nextEdge.to]) {
                            par[nextEdge.to] = v;
                            dist[nextEdge.to] = possibleD;
                            long item = possibleD;
                            item = (item << 32) + nextEdge.to;
                            pq.add(item);
                        }
                    }
                }
        ...
        int c = stop;
        LinkedList<Integer> notret = new LinkedList<>();
        while(c != -1) {
            notret.push(c);
            c = par[c];
        }

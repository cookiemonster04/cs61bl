...
public class TERendererCopy {
     ...
     */
    public void renderFrame(TETile[][] world, String err) {
        ...
        if (mX >= 0 && mX < world.length && mY >= 0 && mY < world[0].length) {
            tileType = world[mX][mY].description();
        }
        StdDraw.setPenColor(StdDraw.WHITE);
        StdDraw.line(0, Engine.HEIGHT, Engine.WIDTH, Engine.HEIGHT);
        StdDraw.textLeft(Engine.WIDTH - 5, Engine.HEIGHT + 1, tileType);

        StdDraw.text(Engine.WIDTH/2, Engine.HEIGHT + 1, err);
        StdDraw.show();
    }
    public void renderFrame(TETile[][] world, String err, boolean toggleSight, int aX, int aY) {
        int numXTiles = world.length;
        int numYTiles = world[0].length;
        StdDraw.clear(new Color(0, 0, 0));
        for (int x = 0; x < numXTiles; x += 1) {
